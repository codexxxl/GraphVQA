...
class PipelineModel(torch.nn.Module):
    def __init__(self):
        ...
        hid_dim = self.question_hidden_dim  * 4 # due to concat
    ...
    def forward(self,
        ...
        # Memory Input: Quesiton and Graph Feature
        ##################################
        # print("graph_final_feature",graph_final_feature.shape, "history_vectors", history_vectors.shape, "questions_encoded", questions_encoded.shape )
        full_answer_feature_memory = torch.cat( (graph_final_feature.unsqueeze(0) , history_vectors, questions_encoded), dim=0)
        # [ Len, Batch ] -> [ Len, Batch, self.question_hidden_dim ]
        if not SAMPLE_FLAG:
            full_answers_output = self.full_answer_decoder(memory=full_answer_feature_memory, tgt=full_answers_input)
        else:
            full_answers_output = self.full_answer_decoder.sample(memory=full_answer_feature_memory, tgt=full_answers_input)

        ##################################
        # Call Short Answer Classification Module Only for Debug
        ##################################
        # short_answer_feature = questions_encoded[0]
        short_answer_feature = torch.cat( (history_vectors[-1], graph_final_feature, questions_encoded[0], graph_final_feature * questions_encoded[0] ), dim=-1 )

        short_answer_logits = self.logit_fc(short_answer_feature)

        return programs_output, full_answers_output, short_answer_logits, execution_bitmap
...
if __name__ == "__main__":
    ...
    for data_batch in data_loader:
        ...
        programs_output, full_answers_output, short_answer_logits, execution_bitmap = output
        print("output: programs_output, full_answers_output, short_answer_logits execution_bitmap \n",
        programs_output.shape,
        full_answers_output.shape,
        short_answer_logits.shape,
        execution_bitmap.shape,
        )
        ...
        programs_output, full_answers_output, short_answer_logits, execution_bitmap = output
        print("output: programs_output, full_answers_output, short_answer_logits \n",
        programs_output.shape,
        full_answers_output.shape,
        short_answer_logits.shape,
        execution_bitmap.shape,
