...
"""
sequence of 5 GINE layers, takes in node features only and ouput the last layer's hidden states
"""

class gine_seq(torch.nn.Module):
    def __init__(self, in_channels, out_channels, ins_dim, dropout=0.0):
        ...
        self.convs = torch.nn.ModuleList([GINEConv(Seq(Lin(in_channels+ins_dim, out_channels), ReLU(), Lin(out_channels, out_channels))) for _ in range(5)])
...
class PipelineModel(torch.nn.Module):
    def __init__(self):
        ...
        # graph excution
        self.gine_seq = gine_seq(in_channels=self.scene_graph_encoder.sg_emb_dim, 
    ...
    def forward(self,
        ...
        # excute the 5 layers of GINE
        x_executed = self.gine_seq(x=x_encoded, edge_index=gt_scene_graphs.edge_index, edge_attr=edge_attr_encoded, instr_vectors=instr_vectors, batch=gt_scene_graphs.batch)
